...
export function draftCompleteness(draft: AuthoringDraft): DraftDiagnostic[] {
	...
	const need = (value: unknown, path: string, tool: string, nonempty = false) => {
		if (value === undefined || (nonempty && (value === "" || (Array.isArray(value) && !value.length)))) diagnostics.push({
			code: "draft_field_missing", category: "incomplete", path, authoringPath: path,
			message: `Specify ${path} before compilation.`, suggestedTool: `workflow_draft_${tool}`,
		});
	};
	...
	for (const node of draft.nodes) {
		...
		for (const key of ["objective", "responsibilities", "non_responsibilities", "work_requirements", "constraints"] as const)
			need(node.contract?.[key], `${path}.contract.${key}`, "configure_nodes", ["objective", "responsibilities", "work_requirements"].includes(key));
		for (const key of ["agent_ref", "participant_id", "tools", "skills", "knowledge_bases", "required_capabilities"] as const)
		...
		if (node.kind === "execution") {
			...
			for (const output of node.outputs) {
				for (const key of ["artifact_type", "description", "business_purpose", "path_prefix", "evidence_requirements", "process_evidence_requirement_refs"] as const)
					need(output[key], `${path}.outputs[${output.output_id}].${key}`, "outputs", typeof output[key] === "string");
				need(draft.criteria.filter((criterion) => criterion.output_bindings.some((ref) => outputKey(ref) === `${node.node_id}/${output.output_id}`)), `${path}.outputs[${output.output_id}].criterion_refs`, "criteria", true);
			}
		} else {
	...
	for (const criterion of draft.criteria) {
		...
		need(definition.evidence_requirements, `${path}.evidence_requirements`, "criteria", definition.kind === "semantic");
		...
		} else if (definition.kind === "semantic") need(definition.process_criterion_refs, `${path}.process_criterion_refs`, "criteria");
	}
