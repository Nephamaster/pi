...
function requireNode(draft: AuthoringDraft, id: string, kind?: DraftNode["kind"]): DraftNode {
	...
	if (!node || (kind && node.kind !== kind))
		throw new DraftError(
			"node_unknown_or_wrong_kind",
			`node:${id}`,
			`Expected a declared ${kind ?? "workflow"} node: ${id}`,
		);
	return node;
}
function addPort(node: DraftNode, id: string, defaults: string[]): DraftOutput {
	if (node.kind !== "execution")
		throw new DraftError("review_output_forbidden", `node:${node.node_id}`, "Only execution nodes own output ports.");
...
function editInput(draft: AuthoringDraft, edit: InputEdit, declaration: boolean): void {
	...
	if (!existing && !edit.source)
		throw new DraftError("input_source_missing", `node:${node.node_id}.inputs`, "A new input needs a source.");
	if (edit.input_id.startsWith("reviewin-") && !existing)
		throw new DraftError(
			"reserved_input_id",
			`node:${node.node_id}.inputs`,
			"reviewin- IDs are derived. Use a distinct explicit target input ID.",
		);
	if (declaration && existing) {
		const source =
			existing.kind === "task_material"
				? { kind: existing.kind, material_id: existing.material_id }
				: { kind: existing.kind, ...existing.source };
		if (hashJson(source) !== hashJson(edit.source))
			throw new DraftError(
				"explicit_reconnect_required",
				`node:${node.node_id}.inputs[${edit.input_id}]`,
				"Use workflow_draft_inputs to reconnect an existing input.",
			);
		return;
	}
	...
	if (source) {
		...
		input =
			source.kind === "task_material"
				? {
						kind: "task_material",
						input_id: edit.input_id,
						material_id: source.material_id,
						...(sameKind && existing ? { required: existing.required } : {}),
					}
				: {
						...(sameKind && existing ? existing : {}),
						kind: "node_output",
						input_id: edit.input_id,
						source: { node_id: source.node_id, output_id: source.output_id },
					};
	} else input = structuredClone(existing!);
	if (edit.required !== undefined) input.required = edit.required;
	if (input.kind === "task_material") {
		if (edit.access !== undefined || edit.purpose !== undefined)
			throw new DraftError(
				"material_policy_invalid",
				`node:${node.node_id}.inputs[${edit.input_id}]`,
				"Task materials have no node-output access or provenance purpose.",
			);
	} else {
		if (edit.access !== undefined) input.access = structuredClone(edit.access);
		if (edit.purpose !== undefined) {
			input.purpose = edit.purpose;
			delete input.omit_default_purpose;
		}
	}
	...
	switch (command.domain) {
		case "topology": {
			...
			for (const item of data.nodes ?? []) {
				...
				if (node && node.kind !== item.kind)
					throw new DraftError(
						"immutable_node_kind",
						`node:${item.node_id}`,
						"Node kind cannot be changed in place.",
					);
				if (!node) {
					node = { node_id: item.node_id, kind: item.kind, name: item.name, inputs: [], outputs: [] };
			...
			for (const item of data.connections ?? []) {
				editInput(draft, item, true);
				changed.push(`node:${item.consumer_node_id}.inputs[${item.input_id}]`);
			}
			for (const id of data.remove_node_ids ?? []) {
				requireNode(draft, id);
				draft.nodes = draft.nodes.filter((node) => node.node_id !== id);
		...
		case "configure_nodes": {
			...
			for (const item of command.data.nodes) {
				...
				if (item.employee || item.resources) {
					if (!node.agent) {
						node.agent = { participant_id: "main", required_capabilities: [], skills: [], knowledge_bases: [] };
						defaults.push(`node:${node.node_id}.agent=main; empty optional resource lists`);
					}
					node.agent = {
						...node.agent,
						...item.employee,
						...item.resources,
						permissions: { external_actions: false, ...node.agent.permissions, ...item.resources?.permissions },
					};
				}
				...
				for (const key of Object.keys(item).filter((key) => key !== "node_id"))
					changed.push(`node:${node.node_id}.${key}`);
			}
			break;
		}
		case "outputs": {
			...
			for (const item of command.data.remove ?? []) {
				const node = requireNode(draft, item.node_id, "execution");
				if (!node.outputs.some((output) => output.output_id === item.output_id))
					throw new DraftError("output_unknown", outputKey(item), "Cannot delete an unknown output.");
		...
		case "criteria": {
			...
			for (const item of command.data.upsert ?? []) {
				...
				if (!kind)
					throw new DraftError(
						"criterion_kind_missing",
						`criterion:${item.criterion_id}`,
						"Declare mechanical or semantic when creating a criterion.",
					);
				if (existing?.definition.kind && existing.definition.kind !== kind)
					throw new DraftError(
						"immutable_criterion_kind",
						`criterion:${item.criterion_id}`,
						"Criterion kind cannot be changed in place.",
					);
				upsert(
					draft.criteria,
					{
						...
						output_bindings: item.output_bindings ?? existing?.output_bindings ?? [],
					},
					(value) => value.criterion_id,
				);
				changed.push(`criterion:${item.criterion_id}`);
			}
			for (const id of command.data.remove_ids ?? []) {
				if (!draft.criteria.some((item) => item.criterion_id === id))
					throw new DraftError("criterion_unknown", id, "Cannot delete an unknown criterion.");
				draft.criteria = draft.criteria.filter((item) => item.criterion_id !== id);
		...
		case "inputs": {
			...
			for (const item of command.data.upsert ?? []) {
				editInput(draft, item, false);
				changed.push(`node:${item.consumer_node_id}.inputs[${item.input_id}]`);
			}
			for (const item of command.data.remove ?? []) {
				const node = requireNode(draft, item.consumer_node_id);
				if (!node.inputs.some((input) => input.input_id === item.input_id))
					throw new DraftError(
						"input_unknown_or_derived",
						item.input_id,
						"Only explicit inputs can be removed; derived review inputs follow assignments.",
					);
		...
		case "reviews": {
			...
			for (const item of command.data.upsert) {
				...
				if (item.assignments)
					unique(item.assignments, (assignment) => assignment.criterion_id, `review:${node.node_id}.assignments`);
		...
		case "stages": {
			...
			for (const item of command.data.upsert ?? []) {
				const current = draft.stage_plans.find((stage) => stage.stage_id === item.stage_id);
				upsert(draft.stage_plans, { ...current, ...item }, (stage) => stage.stage_id);
				changed.push(`stage:${item.stage_id}`);
			}
			for (const id of command.data.remove_ids ?? []) {
				if (!draft.stage_plans.some((stage) => stage.stage_id === id))
					throw new DraftError("stage_unknown", id, "Cannot delete an unknown stage.");
				draft.stage_plans = draft.stage_plans.filter((stage) => stage.stage_id !== id);
		...
		case "governance": {
			const data = command.data;
			if (data.metadata) {
				draft.metadata = { ...draft.metadata, ...data.metadata };
				changed.push("metadata");
			}
			if (data.prerequisites === null) {
				delete draft.prerequisites;
				changed.push("prerequisites");
			} else if (data.prerequisites !== undefined) {
			...
			for (const item of data.decisions?.upsert ?? []) {
				...
				changed.push(`decision:${item.decision_id}`);
			}
			for (const id of data.requirements?.remove_ids ?? []) {
				if (!draft.requirements.some((item) => item.requirement_id === id))
					throw new DraftError("requirement_unknown", id, "Cannot delete an unknown requirement.");
				draft.requirements = draft.requirements.filter((item) => item.requirement_id !== id);
				changed.push(`requirement:${id}`);
			}
			for (const id of data.decisions?.remove_ids ?? []) {
				if (!draft.decisions.some((item) => item.decision_id === id))
					throw new DraftError("decision_unknown", id, "Cannot delete an unknown decision.");
				draft.decisions = draft.decisions.filter((item) => item.decision_id !== id);
		...
		case "coverage": {
			...
			for (const item of command.data.upsert ?? []) {
				upsert(draft.process_coverage, item, key);
				changed.push(`coverage:${key(item)}`);
			}
			for (const item of command.data.remove ?? []) {
				if (!draft.process_coverage.some((value) => key(value) === key(item)))
					throw new DraftError("coverage_unknown", key(item), "Cannot delete an unknown coverage row.");
				draft.process_coverage = draft.process_coverage.filter((value) => key(value) !== key(item));
				changed.push(`coverage:${key(item)}`);
			}
			break;
		}
		case "completion":
			draft.completion = { ...draft.completion, ...command.data };
