...
.scroll-panel{overflow:auto;overscroll-behavior:contain;scrollbar-gutter:stable}.task-card{display:flex;flex-direction:column;min-height:0}.task-markdown{height:310px;min-height:310px;flex:1 1 310px;padding:2px 8px 2px 0;font-size:12px;line-height:1.62;color:#26354d}.task-markdown h1,.task-markdown h2,.task-markdown h3,.task-markdown h4,.task-markdown h5,.task-markdown h6{margin:12px 0 6px;line-height:1.35;color:#172033}.task-markdown h1{font-size:17px}.task-markdown h2{font-size:15px}.task-markdown h3{font-size:13px}.task-markdown p{margin:6px 0}.task-markdown ul,.task-markdown ol{margin:6px 0;padding-left:22px}.task-markdown li{margin:3px 0}.task-markdown code{font:11px/1.5 ui-monospace,SFMono-Regular,Consolas,monospace;background:#edf2f8;border:1px solid #dfe5ee;border-radius:4px;padding:1px 4px}.task-markdown pre{margin:8px 0;padding:10px;border-radius:8px;background:#f1f4f8;border:1px solid #dce3ed;overflow:auto}.task-markdown pre code{padding:0;border:0;background:none}.task-markdown blockquote{margin:8px 0;padding:5px 10px;border-left:3px solid #9fb5da;background:#f7f9fc;color:#526078}.selection-rationale{max-height:190px;margin-top:5px;padding:10px 12px;border:1px solid #e0e5ed;border-radius:9px;background:#f8fafc;white-space:pre-wrap;font-size:11px;line-height:1.65;color:#354259}.selection-details{margin-top:11px}.selection-details summary{cursor:pointer;color:#3f4d63;font-weight:700}.selection-details-body{max-height:220px;overflow:auto;margin-top:8px;padding:10px 12px;background:#f8fafc;border-radius:9px}
...
const STATUS_LABELS={pending:"等待选择",running:"运行中",paused:"已暂停",blocked:"已阻塞",succeeded:"已成功",failed:"已失败",cancelled:"已取消",planned:"待安排",waiting:"等待中",ready:"已就绪",active:"执行中",waiting_review:"等待评审",waiting_rework:"等待返工"};
const EVENT_LABELS={process_selected:"流程已选择",process_staffing_checked:"人员配置已检查",workflow_draft_validated:"工作流草稿已校验",workflow_designed:"工作流已设计",workflow_compiled:"工作流已编译",round_started:"工作轮次开始",round_submitted:"工作轮次已提交",review_recorded:"评审已记录",node_succeeded:"节点已完成",node_blocked:"节点已阻塞",run_succeeded:"运行已成功",run_failed:"运行已失败",run_cancelled:"运行已取消",run_paused:"运行已暂停",run_resumed:"运行已继续",work_progress_saved:"工作进度已保存",resource_cleanup:"资源清理",cleanup_failed:"清理未完成"};
...
function nodeFill(status,kind){if(status==="active")return"#eaf2ff";if(status==="succeeded")return"#e7f7ef";if(status==="blocked"||status==="failed")return"#fff0f0";if(status==="paused"||status==="waiting_rework"||status==="waiting_review")return"#fff8e7";return kind==="review"?"#f5f1ff":"#ffffff"}
function nodeStroke(status,kind){if(status==="active")return"#6ea8fe";if(status==="succeeded")return"#55d187";if(status==="blocked"||status==="failed")return"#ff6b6b";if(status==="paused"||status==="waiting_rework"||status==="waiting_review")return"#e5b454";return kind==="review"?"#8069bf":"#334257"}
...
function renderNodeDetail(node){
  ...
  const environment=node.environment?'<br><b>执行环境：</b>'+esc(node.environment.id+"@"+node.environment.version+" · "+node.environment.provider):"";
  const access='<div class="access"><b>读取范围：</b>'+esc(node.permissions.readPaths.join("、")||"无")+'<br><b>写入范围：</b>'+esc(node.permissions.writePaths.join("、")||"只读")+'<br><b>外部操作：</b>'+(node.permissions.externalActions?"允许":"不允许")+environment+"</div>";
