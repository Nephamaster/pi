...
状态：V1 框架  
Runtime 版本：`1.0.0`  
运行时资产：`prompts/workflow-authoring-guide.md`；TypeScript 只负责加载，不在源码中维护第二份正文。
...
6. 为每个节点定义输入、输出、最小权限和返工目标；bounded Run 需要分配节点预算，unbounded Run 由 Runtime 注入无上限预算；Node `readScopes` 必须覆盖被引用知识库的路径。
...
12. `(Workflow ID, version)` 是不可变资产身份；只有内容完全一致时才能复用。若收到 `workflow_version_conflict`，保持 ID 和其他未受影响分段不变，仅把 Header version 提升到尚未使用的更高 SemVer，再次 finalize。
...
- 同一个 Gate 内 Reviewer 分配全局互斥：一张 AgentCard 只能占一个 Requirement slot。规划时必须确认整个 Requirement 集合存在完整分配，不能把同一员工分别重复计入多项数量检查。
...
- 每个节点提交一个或多个文件，Gate 明确这些文件的机械与语义准出要求；
- 成功依赖无环，Final Artifact 可达；
- 并行节点 Workspace Scope 不冲突；
- Runtime 注入默认 10 次 Attempt，返工目标有效；
...
Compiler 和 Workflow Asset 不变量是确定性裁判。候选被拒绝时，ST 必须根据结构化 Diagnostic 修改 WorkflowDefinition，不得争辩、删除准出标准或绕过不变量。版本冲突定位到 `/version`，因此只允许修改 Header，不应全量重提节点和 Gate。
