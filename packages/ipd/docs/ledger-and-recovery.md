...
| `workflow_versions` | 单 Run 冻结 Workflow JSON、Hash 和来源 |
...
`freezeWorkflow()` 只能在 Run=compiling 时调用，并在一个事务中：
...
V1 不恢复正在流式生成的 AgentSession。进程重新进入 `GraphEngine.run()` 且 Run=running 时：

- 活动阶段 Gate → interrupted；
- running/gate_checking/gate_reviewing Attempt → interrupted；
- 新增恢复 Event，不改写旧记录；
- 只读、无 Bash、无 external action 的节点可从下一 Attempt 重放；
- 有界写节点的文件位于 Attempt Workspace；未通过 Gate 的文件不发布到正式工作区；
- Bash 或 external action 节点仍可能有工作区外副作用，进程中断时进入治理路径；
- 恢复单位始终是完整 Attempt。
...
- 缺少信息、权限或外部依赖；
- write/Bash/external action 节点在进程中断时状态未知；
- Attempt 耗尽时由 Staff 选择 replan/ask_user/fail；冻结 Run 不再创建越界 Attempt；
- Hard Limit 达到；
- Staff 无法自动解决阻塞。
