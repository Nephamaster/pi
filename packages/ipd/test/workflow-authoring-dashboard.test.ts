...
const state = { runId: "run-1", phase: "design", status: "running", revision: 0, nodes: [], rounds: [], events: [] } as unknown as RunState;
const references = { task_input_ref: { id: "task", hash: "a".repeat(64) }, process_selection_ref: { id: "selection", hash: "b".repeat(64) } };

describe("incomplete authoring dashboard", () => {
	it("renders identity-only nodes without inventing contracts or employees", () => {
		const draft = applyDraftCommand(newAuthoringDraft("run-1", references), { domain: "topology", data: {
			nodes: [{ node_id: "A", kind: "execution", name: "Analyze", output_ids: ["data"] }, { node_id: "R", kind: "review", name: "Review" }],
			connections: [{ consumer_node_id: "R", input_id: "candidate", source: { kind: "node_output", node_id: "A", output_id: "data" } }],
		} }).draft;
