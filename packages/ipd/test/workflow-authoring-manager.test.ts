...
async function setup(seed = false) {
	const root = await mkdtemp(join(tmpdir(), "ipd-authoring-manager-")); roots.push(root);
	const base = createCompilerFixture(); let calls = 0;
	const options = {
		...
		validator: (workflow: WorkflowDefinition) => {
			calls++; const result = compileWorkflow({ ...base, workflow });
			return result.ok ? [] : result.report.diagnostics;
		},
	};
	const manager = new WorkflowDraftManager(options);
	await manager.open(base.runId);
	if (seed) for (const [index, command] of authoringCommands(base.workflow).entries()) await manager.edit(index, `seed-${index}`, command);
...
describe("durable authoring and compilation", () => {
	it("returns original edit receipts across later revisions", async () => {
		...
		await manager.edit(1, "rename", { domain: "configure_nodes", data: { nodes: [{ node_id: "A", name: "New name" }] } });
		expect(await manager.edit(0, "declare", skeleton)).toMatchObject({ applied_revision: 1, current_revision: 2, replayed: true });
	...
	it("rejects unknown fields and trusted-reference injection without saving", async () => {
		const { manager } = await setup();
		await expect(manager.edit(0, "bad", { domain: "governance", data: { task_input_ref: { id: "fake", hash: "f".repeat(64) } } })).rejects.toThrow("Invalid domain command");
	...
	it("serializes independent managers against the same revision", async () => {
		...
		const other = new WorkflowDraftManager(options); await other.open(base.runId);
	...
	it("captures exact immutable candidates and reopens only through trusted control", async () => {
		...
		expect(await readFile(join(root, first.receipt.candidate_file!), "utf8")).toBe(`${JSON.stringify(first.workflow, null, "\t")}\n`);
		...
		await manager.edit(state.revision, "fix-contract", { domain: "configure_nodes", data: { nodes: [{ node_id: "produce", contract: { work_requirements: ["Use supplied material and preserve evidence"] } }] } });
	...
	it("does not allow legacy whole-node edits after domain authoring", async () => {
		const { manager } = await setup(); await manager.edit(0, "declare", skeleton);
		await expect(manager.apply((await manager.read()).draftId, 1, "legacy", [{ kind: "remove_node", node_id: "A" }])).rejects.toThrow("Legacy whole-object");
	});
	it("cannot reopen or replay-submit cancelled designer control", async () => {
		const { manager } = await setup(true); const state = await manager.read();
		await manager.submitRevision(state.revision, "submit-1"); await manager.close();
	...
	it("requires stopped-writer migration and retains the original legacy bytes", async () => {
		...
		const { nodes, criteria, requirement_coverage, completion, task_input_ref, process_selection_ref, ...header } = base.workflow;
		const raw = JSON.stringify({ draftId: `${base.runId}:workflow-draft`, runId: base.runId, revision: 4, trustedReferences: { task_input_ref, process_selection_ref }, header, nodes, criteria, requirementCoverage: requirement_coverage, completion, operations: {} });
	...
	it("exposes fourteen domain tools and never returns the full compiled Workflow", async () => {
		...
		const result = await validate.execute("validate", { expected_revision: (await manager.read()).revision }, undefined, undefined, {} as never);
