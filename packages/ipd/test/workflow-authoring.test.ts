...
function legacy(workflow: WorkflowDefinition): LegacyDraft {
	const { nodes, criteria, completion, requirement_coverage, task_input_ref, process_selection_ref, ...header } = workflow;
	return { draftId: "run-1:workflow-draft", runId: "run-1", revision: 5, trustedReferences: { task_input_ref, process_selection_ref }, header, nodes, criteria, requirementCoverage: requirement_coverage, completion, operations: {} };
}
...
describe("partial workflow authoring", () => {
	it("saves an incomplete skeleton without inventing professional content", () => {
		...
		const state = newAuthoringDraft("run-1", { task_input_ref: base.task_input_ref, process_selection_ref: base.process_selection_ref });
		const draft = edit(state, { domain: "topology", data: { nodes: [{ node_id: "A", kind: "execution", name: "Analysis", output_ids: ["result"] }] } });
	...
	it("materializes a complete domain design through the unchanged Compiler", () => {
		...
		expect(workflow.nodes[1].inputs[0]).toMatchObject({ input_id: reviewInputId(ref), source: ref, required: true, purpose: "test_subject", availability: "submitted" });
	...
	it("updates one field without replacing the node, employee or independent reviewer", () => {
		const before = ready();
		const after = edit(before, { domain: "configure_nodes", data: { nodes: [{ node_id: "produce", contract: { work_requirements: ["Submit when sufficient"] } }] } });
	...
	it("keeps an explicitly cleared required array incomplete", () => {
		const after = edit(ready(), { domain: "configure_nodes", data: { nodes: [{ node_id: "produce", contract: { responsibilities: [] } }] } });
	...
	it("clears only an optional environment override", () => {
		let draft = edit(ready(), { domain: "configure_nodes", data: { nodes: [{ node_id: "produce", environment_ref: { id: "general-purpose", version: "1.0.0" } }] } });
		draft = edit(draft, { domain: "configure_nodes", data: { nodes: [{ node_id: "produce", environment_ref: null }] } });
		expect(draft.nodes[0].environment_ref).toBeUndefined();
	});
	it("rolls back a batch and never mutates the caller's state", () => {
		const state = ready(); const original = structuredClone(state);
		expect(() => edit(state, { domain: "configure_nodes", data: { nodes: [{ node_id: "produce", name: "Changed" }, { node_id: "missing", name: "Bad" }] } })).toThrow();
		expect(state).toEqual(original);
	});
	it("rejects duplicate identities and kind replacement", () => {
		expect(() => edit(ready(), { domain: "configure_nodes", data: { nodes: [{ node_id: "produce", name: "A" }, { node_id: "produce", name: "B" }] } })).toThrow();
		expect(() => edit(ready(), { domain: "topology", data: { nodes: [{ node_id: "produce", kind: "review", name: "Replaced" }] } })).toThrow();
	});
	...
	it("requires explicit input use choices and rejects hidden topology reconnection", () => {
		let draft = edit(ready(), { domain: "topology", data: { connections: [{ consumer_node_id: "review-produce", input_id: "explicit", source: { kind: "node_output", ...ref } }] } });
		expect(draftCompleteness(draft).some((item) => item.path.includes("explicit"))).toBe(true);
		draft = edit(draft, { domain: "inputs", data: { upsert: [{ consumer_node_id: "review-produce", input_id: "explicit", required: true, purpose: "test_subject", access: { mode: "review_candidate" } }] } });
		expect(() => edit(draft, { domain: "topology", data: { connections: [{ consumer_node_id: "review-produce", input_id: "explicit", source: { kind: "task_material", material_id: "brief" } }] } })).toThrow(/reconnect/i);
	});
	it("preserves explicit required candidate reads instead of duplicating them", () => {
		const draft = edit(ready(), { domain: "inputs", data: { upsert: [{ consumer_node_id: "review-produce", input_id: "candidate", source: { kind: "node_output", ...ref }, required: true, purpose: "test_subject", access: { mode: "review_candidate" } }] } });
		const workflow = materializeDraft(draft).candidate as WorkflowDefinition;
	...
	it("does not accept an optional explicit target read", () => {
		const draft = edit(ready(), { domain: "inputs", data: { upsert: [{ consumer_node_id: "review-produce", input_id: "candidate", source: { kind: "node_output", ...ref }, required: false, purpose: "test_subject", access: { mode: "review_candidate" } }] } });
		expect(() => materializeDraft(draft)).toThrow(/required/);
	});
	it("rejects reserved generated IDs and execution review-candidate access", () => {
		expect(() => edit(ready(), { domain: "inputs", data: { upsert: [{ consumer_node_id: "review-produce", input_id: reviewInputId(ref), source: { kind: "task_material", material_id: "brief" } }] } })).toThrow();
		expect(() => edit(ready(), { domain: "inputs", data: { upsert: [{ consumer_node_id: "produce", input_id: "bad", source: { kind: "node_output", ...ref }, access: { mode: "review_candidate" } }] } })).toThrow();
	});
	it("does not turn legal rework into a forward edge", () => {
		const state = ready();
		expect(draftTopology(state).edges.some((edge) => edge.from === "review-produce" && edge.to === "produce")).toBe(false);
		expect(() => edit(state, { domain: "inputs", data: { upsert: [{ consumer_node_id: "produce", input_id: "cycle", source: { kind: "node_output", ...ref }, required: true, purpose: "content_basis", access: { mode: "approved", review_node_ids: ["review-produce"] } }] } })).toThrow(/cycle/i);
	});
	it("derives stage uses from the same input record", () => {
		let draft = edit(ready(), { domain: "topology", data: { nodes: [{ node_id: "B", kind: "execution", name: "Consume", output_ids: ["result"] }] } });
		draft = edit(draft, { domain: "stages", data: { upsert: [{ stage_id: "S", member_node_ids: ["produce", "B"], exits: [{ output: ref, gate_node_ids: ["review-produce"] }] }] } });
		draft = edit(draft, { domain: "inputs", data: { upsert: [{ consumer_node_id: "B", input_id: "basis", source: { kind: "node_output", ...ref }, required: true, purpose: "content_basis", access: { mode: "stage_candidate", stage_id: "S" } }] } });
		const workflow = materializeDraft(draft).candidate as WorkflowDefinition;
		expect(workflow.stages?.[0].internal_uses).toEqual([{ consumer_node_id: "B", input_id: "basis" }]);
		expect(() => edit(draft, { domain: "stages", data: { upsert: [{ stage_id: "S", member_node_ids: ["produce"] }] } })).toThrow(/members/);
	});
	it("derives composite subjects without imposing a joint standard on each producer", () => {
		const other = { node_id: "D", output_id: "other" };
		let draft = edit(ready(), { domain: "topology", data: { nodes: [{ node_id: "D", kind: "execution", name: "Independent", output_ids: ["other"] }] } });
		draft = edit(draft, { domain: "criteria", data: { upsert: [{ criterion_id: "joint", definition: { kind: "semantic" }, output_bindings: [] }] } });
		draft = edit(draft, { domain: "reviews", data: { upsert: [{ review_node_id: "review-produce", assignments: [{ criterion_id: "joint", mode: "composite", subjects: [ref, other] }], allowed_rework_node_ids: ["produce", "D"] }] } });
		const workflow = materializeDraft(draft).candidate as WorkflowDefinition;
	...
	it("preserves explicit empty optional fields and single-subject declarations", () => {
		const workflow = createValidWorkflow();
		workflow.stages = []; workflow.requirements = []; workflow.decisions = [];
		const reviewer = workflow.nodes[1];
		if (reviewer.kind !== "review") throw new Error("Unexpected fixture node");
	...
	it("refuses legacy multi-member collapse and missing candidate binding", () => {
		const multi = createValidWorkflow(); multi.nodes[0].agents.push(structuredClone(multi.nodes[0].agents[0]));
		expect(() => importLegacyDraft(legacy(multi))).toThrow(/multi-member/);
		const missing = createValidWorkflow(); missing.nodes[1].inputs = [];
	...
	it("does not fill a legacy missing header with new-run defaults", () => {
		const old = legacy(createValidWorkflow()); delete old.header;
	...
	it("maps diagnostics to the responsible authoring editor", () => {
		...
		const mapped = locateDiagnostic({ path: "/nodes/0/outputs/0/path_prefix", message: "Invalid path" }, output.sourceMap);
	...
	it("pages long Unicode records losslessly and rejects stale cursors", () => {
		const draft = ready(); draft.nodes[0].contract!.objective = "证据🙂".repeat(8000);
		const request = { view: "nodes" as const, node_ids: ["produce"], sections: ["contract" as const] };
		let page = readDraftView(draft, request); let text = ""; let count = 0;
		const first = page;
		while (true) {
		...
		expect(() => readDraftView({ ...draft, revision: draft.revision + 1 }, { ...request, cursor: first.next_cursor! })).toThrow(/cursor/i);
	});
	it("keeps graph and read projections pure", () => {
		const draft = ready(); const original = structuredClone(draft);
		checkDraftLinks(draft, draft); draftTopology(draft); materializeDraft(draft); readDraftView(draft, { view: "topology" });
