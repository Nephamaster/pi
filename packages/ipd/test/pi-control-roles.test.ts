...
describe("native control role bindings", () => {
	...
	async function fixture() {
		...
		const options: PiControlRoleOptions = {
			agentDir: root, workspace: root, sessionDirectory: join(root, "sessions"), modelRuntime, model,
			thinkingLevel: "off", agentCard: base.assets.agentCards[0],
		...
		for (const id of ["method", "task"]) {
			...
			skills.push({ id, baseDir, filePath, hash: await hashSkillPackage(baseDir), source: "test", allowedTools: [], description: "Synthetic control procedure." });
		}
	...
	it("rejects overlapping selection, corrects invalid output, and releases its one native session", async () => {
		...
		const decision = {
			status: "selected", process_spec_id: base.processSpec.process_spec_id,
			process_spec_version: base.processSpec.version, rationale: "Matches the task",
			process_requirement_refs: ["produce"], unresolved_fact_refs: [],
		};
		faux.setResponses([
			fauxAssistantMessage(fauxToolCall("submit_process_selection", { ...decision, process_spec_id: "missing" }), { stopReason: "toolUse" }),
		...
		await expect(selector.select("run", base.taskInput, [base.processSpec])).rejects.toThrow("already has a bound Session");
		...
		expect(events.filter(({ event }) => event.type === "tool_execution_end").map(({ event }) => event.type === "tool_execution_end" && event.isError)).toEqual([true, false]);
	...
	it.each([false, true])("designs and revises in one native Session (business Skill=%s)", async (withSkill) => {
		...
		const manager = new WorkflowDraftManager({ file: join(root, "draft.json"), trustedReferences: { task_input_ref: workflow.task_input_ref, process_selection_ref: workflow.process_selection_ref } });
		const draft = await manager.open("run");
		await manager.apply(draft.draftId, 0, "seed", [
			{ kind: "set_header", header: { schema_version: 3, workflow_id: workflow.workflow_id, workflow_version: workflow.workflow_version, name: workflow.name } },
		...
		const designer = new PiWorkflowDesigner({ optionsForRun: () => options, managerForRun: () => manager, designSkill: skills[0], runSkill: withSkill ? skills[1] : undefined, assetSummary: {}, agentCards: base.assets.agentCards });
		faux.setResponses([
			fauxAssistantMessage("Method loaded."),
			fauxAssistantMessage(fauxToolCall("workflow_draft_submit", { expected_revision: 1, operation_id: "submit-design-1" }), { stopReason: "toolUse" }),
			fauxAssistantMessage(fauxToolCall("workflow_draft_submit", { expected_revision: 1, operation_id: "submit-design-2" }), { stopReason: "toolUse" }),
		]);
		try {
			expect(await designer.design("run", base.taskInput, base.processSelection, base.processSpec)).toEqual(workflow);
			expect(await designer.design("run", base.taskInput, base.processSelection, base.processSpec, ["Recheck the workflow"])).toEqual(workflow);
			expect(faux.state.callCount).toBe(3);
			expect(new Set(events.map((event) => event.sessionId)).size).toBe(1);
			const dispatches = events.flatMap(({ event }) => event.type === "message_end" && event.message.role === "user" ? [JSON.stringify(event.message.content)] : []);
		...
		} finally { await designer.cancelRun("run"); }
	});
	...
	it("authors through domain tools in a single native Session and compiles with the existing engine", async () => {
		...
		const manager = new WorkflowDraftManager({
			...
			trustedReferences: { task_input_ref: base.workflow.task_input_ref, process_selection_ref: base.workflow.process_selection_ref },
			validator: (workflow) => {
		...
		faux.setResponses([
			fauxAssistantMessage("Method loaded."),
			...commands.map((command, index) => fauxAssistantMessage(fauxToolCall(`workflow_draft_${command.domain}`, { expected_revision: index, operation_id: `step-${index}`, ...command.data }), { stopReason: "toolUse" })),
			fauxAssistantMessage(fauxToolCall("workflow_draft_submit", { expected_revision: commands.length, operation_id: "submit-domain-workflow" }), { stopReason: "toolUse" }),
		]);
		const designer = new PiWorkflowDesigner({ optionsForRun: () => options, managerForRun: () => manager, designSkill: skills[0], assetSummary: {}, agentCards: base.assets.agentCards });
		try {
			...
			expect(workflow.nodes[1].inputs[0]).toMatchObject({ purpose: "test_subject", required: true, availability: "submitted" });
			expect(new Set(events.map((event) => event.sessionId)).size).toBe(1);
			expect(events.filter(({ event }) => event.type === "tool_execution_end" && event.isError)).toEqual([]);
		} finally { await designer.cancelRun(base.runId); }
	});
});
